...
- 문제나 개념을 이해하는 데 시각 자료가 유용하기 때문에 그래프로 제시되었음
- 데이터가 일반적인 경우에는 사분위수로 나눠 최상위와 최하위 분위수 사이의 차이를 비교함
- 더 많은 데이터를 수집하기 위해서 두 가지 단계를 추가적으로 착수함
- 첫 번째 단계는 수상 프로그램을 시작함
	- 이를 통해 직원들이 가장 선호하는 매니저를 후보로 추천할 수 있도록 함
- 두 번째 단계는 데이터 그래프에서 최상위와 최하위 분위수에 표시된 매니저를 인터뷰함
	- 인터뷰를 통해 애널리스틱스팀은 성공적인 관리 행동 양상과 그렇지 못한 행동 양상의 차이점을 확인할 수 있었음

### 데이터 애널리스트의 역할과 데이터 기반 의사결정의 중요성을 사례 연구
- 첫 번째 사례는 Google HR팀의 매니저 평가 방법과 그 영향력을 분석
- 두 번째 사례는 비영리단체와 관련된 데이터 분석의 예를 들어 기사의 영향력을 조사 
- 이를 통해 데이터 애널리틱스의 중요성과 실제 적용 사례를 이해하고, 데이터 애널리스트의 다양한 역할을 탐색